...
def main(argv, query_id=None, data_dir='.'):
    ...
    if len(opts.args) != 1:
        logging.error("Search expression required")
    ...
    while True:
        ...
        status = ("%03.1f%% | %d scanned | %d matched | %d results" % (
    ...
    if query_id is not None:
        filename = "{}-{}.csv".format(query_id, sid)
    else:
    ...
    with open(filepath, 'w') as f:
        while True:
            ...
            f.flush()
        f.write('\n')
    logging.info("Result of '%s' has been saved into %s", query_id, filepath)
